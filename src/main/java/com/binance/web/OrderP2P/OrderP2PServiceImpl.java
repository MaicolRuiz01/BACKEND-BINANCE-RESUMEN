...
public class OrderP2PServiceImpl implements OrderP2PService {
	...
	@Autowired
	private AccountBinanceRepository cuentaRepository;
	...
	// accountBinanceRepository.findByName(accountName);
	//
	...
	// List<SaleP2P> salesByAccount =
	// saleP2PRepository.findByBinanceAccount(accountBinance);
	//
	// // Convertir la lista de resultados en un Mapa para búsqueda O(1)
	// Map<String, SaleP2P> saleMap = salesByAccount.stream()
	// .collect(Collectors.toMap(sale -> String.valueOf(sale.getNumberOrder()), sale
	// -> sale));
	//
	...
	// if (sale != null) {
	// order.setNameAccount(sale.getNameAccount()); // Asignar el nombre de la
	// cuenta
	// order.setAccountAmount(sale.getPesosCop()); // Asignar el monto de la cuenta
	// asociada
	...
	private List<OrderP2PDto> getOrderP2pByDate(List<OrderP2PDto> ordenes, LocalDate fecha) {
		return ordenes.stream().filter(order -> isSameDay(order.getCreateTime().toLocalDate(), fecha))
				.collect(Collectors.toList());
	}
	...
	public List<OrderP2PDto> ultimasOrdenes(int cantidad) {
		List<String> cuentas = binanceService.getAllAccountNames();

		return cuentas.stream()
				.flatMap(cuenta -> {
					try {
						List<OrderP2PDto> ordenes = getOrderP2P(cuenta); // últimas por cuenta
						ordenes = getOrderP2pCompleted(ordenes); // solo COMPLETED
						ordenes.forEach(o -> o.setNameAccount(cuenta)); // asignar nombre cuenta
						return ordenes.stream();
					} catch (Exception e) {
						System.err.println("⚠️ Error con " + cuenta + ": " + e.getMessage());
						return Stream.empty();
					}
				})
				.sorted((o1, o2) -> o2.getCreateTime().compareTo(o1.getCreateTime())) // ordenar por fecha DESC
				.limit(cantidad)
				.collect(Collectors.toList());
	}
...
public List<OrderP2PDto> getUltimasOrdenesTodas(int cantidad) {
    ...
    return cuentas.stream()
            .flatMap(cuenta -> {
                try {
                    List<OrderP2PDto> ordenes = getOrderP2P(cuenta); // ← no se filtra
                    ordenes.forEach(o -> o.setNameAccount(cuenta)); // asignar nombre cuenta
                    return ordenes.stream();
                } catch (Exception e) {
                    System.err.println("❌ Error con " + cuenta + ": " + e.getMessage());
                    return Stream.empty();
                }
            })
            .sorted((o1, o2) -> o2.getCreateTime().compareTo(o1.getCreateTime())) // por fecha DESC
            .limit(cantidad)
            .collect(Collectors.toList());
}
