...
public class SaleP2PController {
	...
	/**
	 * Devuelve, por cada nameAccount que tenga ventas sin AccountCop asignada, la
	 * suma de pesosCop y la suma de commission.
	 */
	@GetMapping("/s4")
	public ResponseEntity<List<SaleP2PSummaryDto>> getSummaryByName() {
		...
		Map<String, List<SaleP2P>> grouped = all.stream()
				.filter(sale -> (sale.getAccountCops() == null || sale.getAccountCops().isEmpty())
						&& sale.getNameAccount() != null && !sale.getNameAccount().trim().isEmpty())
				.collect(Collectors.groupingBy(SaleP2P::getNameAccount));

		// 3) Construyo la lista de DTOs de resumen
		List<SaleP2PSummaryDto> summary = grouped.entrySet().stream().map(e -> {
			String name = e.getKey();
			double totalPesos = e.getValue().stream().mapToDouble(SaleP2P::getPesosCop).sum();
			double totalComm = e.getValue().stream().mapToDouble(SaleP2P::getCommission).sum();
	...
	public static class SaleP2PSummaryDto {
		...
		public Double getTotalPesosCop() {
			return totalPesosCop;
		}
